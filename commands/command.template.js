...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((options) =>
            options
                .setName('NAME')
                .setDescription("[OPTIONAL / REQUIRED] DESCRIPTION")
    ...
    async execute(client, interaction) {
        await Log("read", interaction.guild.id, `'${interaction.user.tag}' executed '/COMMAND'.`, 'INFO'); //Logs
        ...
        if(!interaction.member.roles.cache.find(role => role.name == MINIMUM_EXECUTION_ROLE)) {
            ...
            await Log("read", interaction.guild.id, `└─'${interaction.user.id}' did not have the required role to use '/NAME'.`, 'WARN'); // LOgs
            return;
        }
