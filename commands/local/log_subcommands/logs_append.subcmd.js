...
module.exports = async function (client, interaction, is_ephemeral, string, object) {
    await Log("read", interaction.guild.id, `└─'${interaction.user.tag}' executed '/log append'.`, 'INFO'); // Logs
    ...
    if(!interaction.member.roles.cache.find(role => role.name == MINIMUM_EXECUTION_ROLE)) {
        ...
        await Log("read", interaction.guild.id, `  └─'${interaction.user.id}' did not have the required role to use '/log'.`, 'WARN');
        return;
    }
    ...
    await Log("read", interaction.guild.id, string, 'LOG'); //Logging
